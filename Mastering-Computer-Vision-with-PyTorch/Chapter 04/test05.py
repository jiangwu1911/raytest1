...
with torch.no_grad():
    for data in testloader:
        ...
        outputs = net(images)
        _, predicted = torch.max(outputs.data, 1)
        y_pred.extend(predicted.cpu().numpy())  # 移回CPU进行numpy转换
        y_true.extend(labels.cpu().numpy())     # 移回CPU进行numpy转换
...
plt.savefig('confusion_matrix.png')  # 保存图像而不是显示
plt.close()
...
with torch.no_grad():
    for data in testloader:
        ...
        all_labels.extend(labels.cpu().numpy())  # 移回CPU
        all_preds.extend(predicted.cpu().numpy())  # 移回CPU
...
with torch.no_grad():
    for data in testloader:
        ...
        # Store predictions, labels and output scores for metrics
        all_labels.extend(labels.cpu().numpy())  # 移回CPU
        all_outputs.extend(outputs.cpu().numpy())  # 移回CPU
...
for i in range(10):
    ...
    plt.plot(fpr[i], tpr[i], label=f'ROC curve of class {classes[i]} (area = {roc_auc_dict[i]:.2f})')
...
plt.savefig('roc_curve.png')  # 保存图像而不是显示
plt.close()
